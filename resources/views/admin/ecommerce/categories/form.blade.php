...
<div class="card card-primary card-outline mb-4">
    <div class="card-header d-flex justify-content-end align-items-center">
        <a href="{{ route('admin.product-categories.index') }}" class="btn btn-primary btn-sm">+ Back to List</a>
    </div>

    <div class="card-body">
        <form action="{{ isset($category) && $category->exists ? route('admin.product-categories.update', $category->id) : route('admin.product-categories.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row">
                ...
                <div class="mb-3 col-md-6">
                    ...
                    @if(isset($category) && $category->banner)
                        <img src="{{ asset('storage/'.$category->banner) }}" alt="{{ $category->alt ?? '' }}" class="img-thumbnail mt-2" width="150">
                    @endif
                </div>
            ...
            <button type="submit" class="btn btn-primary">{{ isset($category) && $category->exists ? 'Update' : 'Create' }}</button>
        </form>
    </div>
</div>
@endsection

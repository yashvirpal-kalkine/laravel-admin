...
<div class="card card-primary card-outline mb-4">
    ...
    <div class="card-body">
        <form
            ...
            <div class="row">
                ...
                <div class="mb-3 col-md-6">
                    ...
                    <input type="text" name="starts_at" class="form-control datetime"
                        value="{{ old('starts_at', isset($coupon->starts_at) ? $coupon->starts_at->format('Y-m-d H:i') : '') }}">
                </div>
                <div class="mb-3 col-md-6">
                    <label class="form-label">Valid Until</label>
                    <input type="text" name="expires_at" class="form-control datetime"
                        value="{{ old('expires_at', isset($coupon->expires_at) ? $coupon->expires_at->format('Y-m-d H:i') : '') }}">
                </div>
                    ...
                    @if(old('actions', $coupon->actions ?? []))
                        @foreach(old('actions', $coupon->actions ?? []) as $i => $action)
                            <tr>
                                <td>
                                    <select name="actions[{{ $i }}][action]" class="form-select action-type">
                                        @foreach(['fixed_discount', 'percentage_discount', 'free_product', 'discount_product', 'bogo'] as $a)
                                            <option value="{{ $a }}" @selected(($action['action'] ?? $action->action ?? '') == $a)>{{ ucfirst(str_replace('_',' ', $a)) }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                ...
                                <td>
                                    <input type="number" step="0.01" name="actions[{{ $i }}][value]" class="form-control mb-1" placeholder="Value" value="{{ $action['value'] ?? $action->value ?? '' }}">
                                    <input type="number" name="actions[{{ $i }}][quantity]" class="form-control" placeholder="Quantity" value="{{ $action['quantity'] ?? $action->quantity ?? '' }}">
                                </td>
                                <td>
                                    <input type="number" name="actions[{{ $i }}][buy_qty]" class="form-control mb-1" placeholder="Buy Qty" value="{{ $action['buy_qty'] ?? $action->buy_qty ?? '' }}">
                                    <input type="number" name="actions[{{ $i }}][get_qty]" class="form-control" placeholder="Get Qty" value="{{ $action['get_qty'] ?? $action->get_qty ?? '' }}">
                                </td>
                                <td>
            ...
            {{-- Submit --}}
            <div class="mt-4">
                <button type="submit" class="btn btn-primary">{{ isset($coupon) && $coupon->exists ? 'Update' : 'Create' }}</button>

...
{{-- SINGLE PRODUCT PAGE --}}
@if($isSingle)
    <button type="button" id="addCart{{ $product->id }}" class="add-to-cart-btn" onclick="addToCart({{ $product->id }})" {{ $inCart ? 'disabled' : '' }}>
        {{ $inCart ? 'Already in Cart' : 'Add to Cart' }}
    </button>

    <button type="button" id="buyNow{{ $product->id }}" class="add-to-cart-btn"
        onclick="addToCart({{ $product->id }}, 1, true)" {{ $inCart ? 'disabled' : '' }}>
        Buy it now
    </button>
    {{-- SHOP / LISTING PAGE --}}
@else
    ...
    @else
        <button class="btn-cart {{ $inCart ? 'in-cart' : '' }} addCart{{ $product->id }}" type="button"
            id="addCart{{ $product->id }}" onclick="addToCart({{ $product->id }})" {{ $inCart ? 'disabled' : '' }}
